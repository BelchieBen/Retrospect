import HelixPalette from "~/styles/palette";

export const ITableAddRowBottom = ({
  color = HelixPalette.neutral90,
  dataId,
  size = 24,
  style,
}: {
  color?: string;
  dataId?: string;
  size?: number;
  style?: React.CSSProperties;
}) => {
  return (
    <svg
      data-id={dataId}
      fill={color}
      width={size}
      height={size}
      viewBox="0 0 24 24"
      style={style}
    >
      <path d="M3 6H6V3H3V6Z"></path>
      <path d="M3 16H6V13H3V16Z"></path>
      <path d="M21.9995 21.9998L17.0005 19.4998L21.9995 16.9998V21.9998Z"></path>
      <path d="M16 21H3V18H16V21Z"></path>
      <path d="M11 16H8V13H11V16Z"></path>
      <path d="M13 16H16V13H13V16Z"></path>
      <path d="M21 16H18V13H21V16Z"></path>
      <path d="M11 6H8V3H11V6Z"></path>
      <path d="M13 6H16V3H13V6Z"></path>
      <path d="M21 6H18V3H21V6Z"></path>
      <path d="M3 11H6V7.99899H3V11Z"></path>
      <path d="M11 11H8V8H11V11Z"></path>
      <path d="M13 11H16V8H13V11Z"></path>
      <path d="M21 11H18V8H21V11Z"></path>
    </svg>
  );
};

export default ITableAddRowBottom;
